...
public:
    ...
    void computeInverse( matrix_mxm< M, float_t >& inverse ) const;
...
template< size_t M, typename float_t >
void
matrix_mxm< M, float_t >::computeInverse( matrix_mxm< M, float_t >& Minverse ) const
{
    throw "not implemented yet.";
}
...
// specializations for M = 2 and M = 3
...
public: 
    ...
    void computeInverse( matrix_mxm< 2, float_t >& Minverse ) const
    {
        const array< array< float_t, 2 >, 2 >& rows = matrix_mxn< 2, 2, float_t >::_rows;
        const float_t& a = rows[ 0 ][ 0 ];
        const float_t& b = rows[ 0 ][ 1 ];
        ...
        float_t reciprocal_of_determinant = 1.0 / computeDeterminant();
